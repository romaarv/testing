...
class TestingPasswordResetCompleteView(PasswordResetCompleteView):
    ...
    title = (_('Пароль успешно сброшен'))
...
class ChangeUserInfoView(SuccessMessageMixin, LoginRequiredMixin, UpdateView):
    ...
    success_message = _('Личные данные пользователя изменены')
...
class TestingPasswordChangeView(SuccessMessageMixin, LoginRequiredMixin, PasswordChangeView):
    ...
    success_url = reverse_lazy('main:profile')
    success_message = _('Пароль пользователя изменен')
...
class DeleteUserView(LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        logout(request)
        messages.add_message(request, messages.SUCCESS, _('Пользователь удален'))
...
def testing(request, task_id):
    ...
    if test.exists():
        ...
        test = Test(task=Task(id=task_id), user=AdvUser(id=request.user.id), variant=variant)
        test.save()
        test = Test.objects.filter(task=task_id, user=request.user.id)
